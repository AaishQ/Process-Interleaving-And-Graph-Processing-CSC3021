# Process-Interleaving-And-Graph-Processing-CSC3021
QUB CONCURRENT - CSC3021 Assignment 1 


**The provided instructions for the assignment by the lecturer are set out below - The code in this repo is for demonstration only and consists of code BUILT UPON via the module:**

This directory holds code files for the graph processing problem.
It implements routines to solve the PageRank problem and the Connected Components problem using label propagation. The latter is valid only for undirected graphs (a.k.a simple graphs).
...
The file format of these, after decompression, is:
CSR and CSC:
...
% java Driver (pr|cc) 1 /path/to/outputfile.txt COO  
/path/to/graph.CSR 
/path/to/graph.CSC 
/path/to/graph.COO
